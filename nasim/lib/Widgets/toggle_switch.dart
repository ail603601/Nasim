...
class _ToggleSwitchState extends State<ToggleSwitch> with AutomaticKeepAliveClientMixin<ToggleSwitch> {
  ...
  Widget build(BuildContext context) {
    ...
    activeBgColor = widget.activeBgColor == null ? [Theme.of(context).primaryColor] : widget.activeBgColor;

    /// Assigns active foreground color to default accent text theme color if it's null/not provided.
    activeFgColor = widget.activeFgColor == null ? Theme.of(context).primaryTextTheme.bodyText1!.color : widget.activeFgColor;

    /// Assigns inactive background color to default disabled theme color if it's null/not provided.
    inactiveBgColor = widget.inactiveBgColor == null ? Theme.of(context).disabledColor : widget.inactiveBgColor;

    /// Assigns inactive foreground color to default text theme color if it's null/not provided.
    inactiveFgColor = widget.inactiveFgColor == null ? Theme.of(context).textTheme.bodyText1!.color : widget.inactiveFgColor;

    /// Assigns transparent border color if it's null/not provided.
    borderColor = widget.borderColor == null ? [Colors.transparent] : widget.borderColor;
    ...
    borderWidth = widget.borderWidth == null ? (widget.borderColor == null ? 0.0 : 3.0) : widget.borderWidth;
              ...
              if (index % 2 == 1) {
                ...
                if (index == 0) {
                  ...
                  cornerRadius = widget.textDirectionRTL
                      ? BorderRadius.only(topRight: Radius.circular(widget.cornerRadius), bottomRight: Radius.circular(widget.cornerRadius))
                      : BorderRadius.only(topLeft: Radius.circular(widget.cornerRadius), bottomLeft: Radius.circular(widget.cornerRadius));
                }
                ...
                var icon = widget.icons != null && widget.icons![index ~/ 2] != null
                    ? Icon(
                        ...
                        color: fgColor,
                        size: widget.iconSize > (_calculateWidth(index ~/ 2) / 3) ? (_calculateWidth(index ~/ 2)) / 3 : widget.iconSize,
                      )
                    : Container();
                ...
                var textStyle = widget.customTextStyles != null && widget.customTextStyles![index ~/ 2] != null
                ...
                return GestureDetector(
                  ...
                  child: AnimatedContainer(
                    ...
                    constraints: BoxConstraints(maxWidth: _calculateWidth(index ~/ 2)),
                    alignment: Alignment.center,
                    decoration: BoxDecoration(
                      borderRadius: widget.radiusStyle ? BorderRadius.all(Radius.circular(widget.cornerRadius)) : cornerRadius,
                      gradient: LinearGradient(
  ...
  double _calculateWidth(int index) {
    ...
    return (widget.totalSwitches + extraWidth) * (widget.customWidths != null ? widget.customWidths![index] : widget.minWidth) < screenWidth
        ? (widget.customWidths != null ? widget.customWidths![index] : widget.minWidth)
